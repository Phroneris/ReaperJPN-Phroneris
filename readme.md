...
5. REAPER の言語を手動設定  
   万が一上記の作業を終えても REAPER が自動的に日本語表示にならない場合、  
   REAPER を再起動した後に環境設定から言語を手動で選択します。  
...
+ REAPER をポータブルインストールして試す  
  ...
  例えば複雑なカスタマイズを試したい場合、まずはポータブル版で挙動を確かめてから通常版にも適用する、  
  といった使い方ができるため大変便利です。  
...
#### このパッチのために、あらかじめ導入が必須なものはある？

ありません。  
[ちえ氏のパッチ][2]には .NET Framework 4.5 が必要ですが、私のはそんな高度なものじゃないです。  
あ、もちろん、製品版 REAPER 本体はインストールしておいてください。  


#### このパッチの実行時に、このパッチを REAPER のインストールフォルダに置く必要はある？

ありません。  
もちろんそういう場所に置いて実行しても構いませんが、基本的にはどこに置いても変わらないはずです。  
[Jamuobasan 氏の REAPER v0.999 用パッチ][2.1]ではその必要がありましたが、私のは方式が全く違うものです。  
...
#### 既にある、ちえ氏の [REAPER 日本語化パッチ][2] との違いは？

それの翻訳をより進めたものです。  
2015/10/10 時点の[ちえ氏のパッチ][2]に対して、独自ツールで中身の見づらさを解消・整理し、  
ついでに[後正面氏のサイズ修正][3]も採り入れて作成したものが元になっています。  
...
* 自己責任でご利用ください。  
* 時々でいいので、最新のバージョンをチェックしてください。  
* 何かあったら、**マジで**お気軽にご連絡ください。  
...
[2.1]: http://freedtm.3rin.net/daw%E7%B4%B9%E4%BB%8B/reaper%200.999
[3]: http://positiveallergy.blog50.fc2.com/blog-entry-723.html
[4]: https://forest.watch.impress.co.jp/library/software/lhaplus/
